...
### Completed batch
- Upgraded the `System` tab’s menu bar settings preview so the new Network item shows live abbreviated download/upload rates instead of a placeholder label.
- Verified the follow-up against the exact staged patch in a detached worktree with a fresh full `xcodebuild ... test` pass.
